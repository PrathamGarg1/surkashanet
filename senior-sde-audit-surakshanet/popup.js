...
function row(item) {
  ...
  div.className = 'row';
  div.innerHTML = `<div class="meta"><span class="${item.severity}">${item.severity}</span> · ${(item.score * 100).toFixed(0)}% · ${new Date(item.ts).toLocaleString()} · ${item.source}</div><div></div>`;
